...
//- PUBLIC & INTERNAL METHODS -----------------------------------------------------------------------------
		...
		public function playSound($name:String, $volume:Number = 1, $startTime:Number = 0, $loops:int = 0):void
		{
			...
			snd.volume = $volume;
			snd.startTime = $startTime + 0.5;
			snd.loops = $loops;
			//trace(snd.volume);
			if (snd.paused)
			{
				snd.channel = snd.sound.play(snd.position, snd.loops, new SoundTransform(snd.volume * _masterVolume));
			}
			else
			{
				snd.channel.stop();
				snd.channel = snd.sound.play($startTime, snd.loops, new SoundTransform(snd.volume * _masterVolume));
			}
			...
			for (var i:int = 0; i < this._sounds.length; i++)
			{
				...
				if ($useCurrentlyPlayingOnly)
				{
					if (this._soundsDict[id].pausedByAll)
					{
						...
						trace(this._soundsDict[id].channel + " is pos in playallsounds");
						this.playSound(id,this._soundsDict[id].volume);
					}
				}
				else
				{
		...
		{
			for (var i:int = this._sounds.length -1; i >=0; i--)
			{
				var id:String = this._sounds[i].name;
				if (getSoundPosition(id) >= getSoundDuration(id))
				{
					this._sounds.splice(i, 1);
					continue;
				}
				...
				if ($useCurrentlyPlayingOnly)
				{
					if (!this._soundsDict[id].paused)
					{
						trace(this._soundsDict[id].channel + " is pos in pauseallsounds");
						this._soundsDict[id].pausedByAll = true;
						this.pauseSound(id);
					}
				}
		...
		/**
		 * Sets the master volume for the entire manager.
		 * 
		 * @param	$volume The volume multiplier, between 0 and 10
		 * @return void
		 */
