define(['Class', 'SpriteLibrary'],function(Class, SpriteLibrary){
	...
	AnimationStateMachine.prototype.popState = function(all){
		var i;
		(all)? i= this.stateList.length-1 : i=1;
		for(i;i>0;i--){
			this.stateList.pop();
		}
	...
	AnimationStateMachine.prototype.parseTransVars = function()
	{
		...
		var curLength = 0;
		var potentialStates = [];
		for (iState in this._stateVars) 
		{
			isMatch = true;
			for (iVar in this._stateVars[iState])
			{
		...
		for (iState2 in potentialStates) 
		{
			length = 0;
			//console.log(potentialStates);
			for (iVar2 in potentialStates[iState2])
			{
				length++;
			}
			if (length >= curLength)
